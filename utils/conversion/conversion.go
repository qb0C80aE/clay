...
func ToStringInterface(value interface{}) (interface{}, error) {
	switch value.(type) {
	case int:
	...
	case float32:
		return strconv.FormatFloat(float64(value.(float32)), 'E', -1, 32), nil
	case float64:
		return strconv.FormatFloat(float64(value.(float64)), 'E', -1, 64), nil
	case bool:
		return strconv.FormatBool(value.(bool)), nil
	default:
		return nil, errors.New("value is not int, float, or boolean")
	}
}
