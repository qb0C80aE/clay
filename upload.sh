...
curl -X DELETE -H "AuthKey: ${AUTH_KEY}" https://clay-download.herokuapp.com/clay.bastet.darwin-amd64.tgz
curl -X DELETE -H "AuthKey: ${AUTH_KEY}" https://clay-download.herokuapp.com/clay.bastet.darwin-386.tgz
...
curl -X PUT -H "AuthKey: ${AUTH_KEY}" -H "Content-Type: multipart/form-data" https://clay-download.herokuapp.com/clay.bastet.darwin-amd64.tgz -F content=@clay.bastet.darwin-amd64.tgz -F description="Clay binary for bastet:Mac OS X/x86_64"
curl -X PUT -H "AuthKey: ${AUTH_KEY}" -H "Content-Type: multipart/form-data" https://clay-download.herokuapp.com/clay.bastet.darwin-386.tgz -F content=@clay.bastet.darwin-386.tgz -F description="Clay binary for bastet:Max OS X/x86"
